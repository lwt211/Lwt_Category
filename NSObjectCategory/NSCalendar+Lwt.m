...
+(NSMutableArray*)switchDay
{
    ...
    NSLog(@"%zd , %%zd", head, foot);
    
    for (int i = -head; i < 0; i++)
    {
        NSString* str = [NSString stringWithFormat:@"%zd", [self getDayWithDate:[self dateSinceNowWithInterval:i]]];
        [array addObject:str];
    }
    
    [array addObject:[NSString stringWithFormat:@"%zd", [self getDayWithDate:[self dateSinceNowWithInterval:0]]]];
    
    //sy 添加日期
    for (int i = 0; i < foot; i++)
    {
        NSString* str = [NSString stringWithFormat:@"%zd", [self getDayWithDate:[self dateSinceNowWithInterval:i+1]]];
        [array addObject:str];
    }
